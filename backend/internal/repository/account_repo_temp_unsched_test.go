...
func TestAccountRepository_UpdateGrokOAuthCredentialsIfUnchanged_UsesExactAttemptStateAndAtomicOutbox(t *testing.T) {
	...
	require.Len(t, exec.execQueries, 1)
	normalized := normalizeSQLWhitespace(exec.execQueries[0])
	require.Contains(t, normalized, "WITH updated_source AS")
	...
	require.Contains(t, normalized, "updated_siblings AS")
	require.Contains(t, normalized, "a.credentials ->> 'sub' = $5::jsonb ->> 'sub'")
	require.Contains(t, normalized, "a.credentials -> '_token_version' IS NOT DISTINCT FROM $5::jsonb -> '_token_version'")
	require.Contains(t, normalized, "INSERT INTO scheduler_outbox")
