...
func TestWriteOpenAIUpstreamProvenanceEmitsExactlyOneValuePerCMSContractHeader(t *testing.T) {
	t.Parallel()

	rec, c := newUpstreamProvenanceTestContext(t, CompositeRouteDecision{})
	// Simulate an upstream trying to inject duplicate authoritative values before
	// the gateway writes its attestation. The CMS client rejects anything other
	// than exactly one value for each of these 12 headers.
	for _, name := range authoritativeUpstreamProvenanceHeaders {
		rec.Header().Add(name, "spoof-one")
		rec.Header().Add(name, "spoof-two")
	}
	...
	for _, name := range []string{
		...
		require.Len(t, rec.Header().Values(name), 1, name)
		require.NotEmpty(t, rec.Header().Values(name)[0], name)
		require.NotContains(t, rec.Header().Values(name), "spoof-one", name)
		require.NotContains(t, rec.Header().Values(name), "spoof-two", name)
	}
}
