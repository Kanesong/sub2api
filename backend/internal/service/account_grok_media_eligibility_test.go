...
func TestGrokMediaGenerationEligibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown JWT tier remains inconclusive", account: &Account{Platform: PlatformGrok, Type: AccountTypeOAuth, Credentials: map[string]any{"subscription_tier": "unknown"}, Extra: map[string]any{grokBillingExtraKey: &xai.BillingSummary{StatusCode: http.StatusOK, Partial: true}}}, want: false, wantReason: "billing_inconclusive"},
